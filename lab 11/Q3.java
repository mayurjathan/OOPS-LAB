...
class MatrixDemo {
    public static void main(String[] args) throws InterruptedException {
        ...
        displayThread.start();
        if (displayThread.isAlive()) System.out.println("\nDisplay thread is alive\n");
        displayThread.join();
        
        transposeThread.start();
        if (transposeThread.isAlive()) System.out.println("\nTranspose thread is alive\n");
        transposeThread.join();
        
        maxValueThread.start();
        if (maxValueThread.isAlive()) System.out.println("\nMaxValue thread is alive\n");
        maxValueThread.join();
        
        principalDiagonalThread.start();
        if (principalDiagonalThread.isAlive()) System.out.println("\nPrincipal Diagonal thread is alive\n");
        principalDiagonalThread.join();
        
        nonDiagonalThread.start();
        if (nonDiagonalThread.isAlive()) System.out.println("\nNon-Diagonal thread is alive\n");
